...
import SetlistSimplePDF from "./PDFs/SetlistSimplePdf";
import SetlistDetailedPDF from "./PDFs/SetlistDetailedPdf";
...
export default function SetlistExportCard({
  ...
  const downloadLink = (document: React.ReactElement<any>, downloadText: string) => (
      ...
      {({ url, loading, error }) =>
        loading ? (
          ...
          <a rel="noopen noreferer" target="_blank" href={url!}>
            <Button disabled={!!error} className="w-full border" variant={"secondary"}>
              {downloadText}
            </Button>
          </a>
        )
  ...
  const lookupSet = (set: setSpot[]) =>
    set.sort((a, b) => a.spotPrio - b.spotPrio).map((spot) => songs.get(spot.songId)!);
  ...
  return (
    <Card className="z-10 bg-gray-900 w-full max-w-sm fixed top-[50%] left-[50%] transform-(--center-transform)">
      <CardHeader>
        ...
        <CardDescription>
          Export your setlist to a a simple stage <code>.pdf</code>, a detailed FOH-ready <code>.pdf</code>,
          or download the <code>.json</code> dump.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-4">
            ...
            <div className="w-full flex flex-row justify-between">
              Wrap short sets:
              <Checkbox checked={wrap} onCheckedChange={(c) => setWrap(Boolean(c))} />
            </div>
            {downloadLink(
              <SetlistSimplePDF
